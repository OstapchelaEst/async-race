...
export class RenderPage {
    ...
    renderTrack(name: string, color: string, id: number) {
        (<HTMLElement>document.querySelector('.garage')).innerHTML += `
      

<div class="track-car" id="${id}">
   <div class="track-car__head">
      <input type="checkbox" name="car" class="track-car__select-car" id="car-${id}">
      <label for="car-${id}" class="track-car__label-car-select">Select</label>
      <button class="track-car__remove-button">Remove</button>
      <p class="track-car__name">${name}</p>
   </div>
   <div class="track-car__body">
      <input type="checkbox" class="track-car__engine" id="engine-car-${id}">
      <label for="engine-car-${id}" class="track-car__label-engine">A</label>
      <input type="checkbox" class="track-car__drive" id="drive-car-${id}">
      <label for="drive-car-${id}" class="track-car__label-drive">B</label>
      <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="60.000000pt" height="25.000000pt"
    ...
    async getTrack() {
        NEW_CARS.getCars().then((data) => {
            data.car.map((e: { name: string; color: string; id: number }) => this.renderTrack(e.name, e.color, e.id));
